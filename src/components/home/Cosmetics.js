...
const Cosmetics = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:8000/Beauty"); // Replace with your JSON server URL and endpoint
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      >
        <h4 style={{ padding: "12px" }}>Best Selling Beauty</h4>

        <div style={{ display: "flex" }}>
          {beauty.map((electronics) => (
            <Grid
              container
              spacing={2}
